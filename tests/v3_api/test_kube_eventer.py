...
def test_sink_elasticsearch_required():
    ...
    sink_cmds,app  = enable_kube_eventer_catalog(answners,False)
...
def test_sink_elasticsearch_all():
    answners = {
        "sinktarget": "elasticsearch",
        "sink.elasticsearch.es_server_url": "nodes=http://foo.com:9200&nodes=http://bar.com:9200",
        "sink.elasticsearch.index": "all",
        "sink.elasticsearch.esUserName": "admin",
        "sink.elasticsearch.esUserSecret": "Harbor12345",
        "sink.elasticsearch.maxRetries": "10",
        "sink.elasticsearch.healthCheck": "/test",
        "sink.elasticsearch.sniff": "2",
        "sink.elasticsearch.startupHealthcheckTimeout": "10",
        "sink.elasticsearch.ver": "ver",
        "sink.elasticsearch.bulkWorkers": "wk1",
        "sink.elasticsearch.cluster_name": "k8s",
        "sink.elasticsearch.pipeline": "test"
    }

    sink_cmds, app = enable_kube_eventer_catalog(answners,False)
    sink_cmd = "--sink=elasticsearch:nodes=http://foo.com:9200&nodes=http://bar.com:9200?index=all&esUserName=admin&esUserSecret=Harbor12345&maxRetries=10&healthCheck=/test&sniff=2&startupHealthcheckTimeout=10&ver=ver&bulkWorkers=wk1&cluster_name=k8s&pipeline=test"
    assert sink_cmd in sink_cmds
...
def test_sink_kafka_required():
    ...
    sink_cmds, app = enable_kube_eventer_catalog(answners,False)
    sink_cmd = "--sink=kafka:?brokers=localhost:9092&brokers=localhost:9093"
    assert sink_cmd in sink_cmds
...
def test_sink_kafka_all():
    answners = {
        "sinktarget": "kafka",
        "sink.kafka.brokers": "brokers=localhost:9092&brokers=localhost:9093",
        "sink.kafka.eventstopic": "test-kafka",
        "sink.kafka.compression": "all",
        "sink.kafka.user": "admin",
        "sink.kafka.password": "Harbor12345",
        "sink.kafka.cacert": "mycacert",
        "sink.kafka.cert": "mycert",
        "sink.kafka.key": "mykey",
        "sink.kafka.insecuressl": False
    }

    sink_cmds, app = enable_kube_eventer_catalog(answners,False)
    sink_cmd = "--sink=kafka:?brokers=localhost:9092&brokers=localhost:9093&eventstopic=<nil>&compression=<nil>&user=&password=<nil>&cacert=<nil>&cert=<nil>&key=<nil>"
    assert sink_cmd in sink_cmds
...
def test_sink_mysql():
    ...
    sink_cmds, app = enable_kube_eventer_catalog(answners)
    sink_cmd = "--sink=mysql:?root:root@tcp(172.1.2.3:3306)/kube_event?charset=utf8"
    assert sink_cmd in sink_cmds
...
def test_sink_wechat_all():
    answners = {
        ...
        "sink.wechat.corp_secret": "12389567weqe",
        "sink.wechat.agent_id": "12389567",
        "sink.wechat.to_user": "admin",
        "sink.wechat.label": "test-wechat",
        "sink.wechat.level": "info",
        "sink.wechat.namespaces": "default,kube-system",
        "sink.wechat.kinds": "Pod"
    }

    sink_cmds,app = enable_kube_eventer_catalog(answners)
    sink_cmd = "--sink=wechat:?corp_id=a12389567&corp_secret=12389567weqe&agent_id=12389567&to_user=admin&label=test-wechat&level=info&namespaces=default,kube-system&kinds=Pod"
    assert sink_cmd in sink_cmds
...
def test_sink_webhook_required():
    answners = {
    "sinktarget": "webhook",
    "sink.webhook.webhook_url": "https://oapi.dingtalk.com/robot/send?access_token=4b4de1354b290410523dcdf3ab04a5b3775f799e8d54949e7952ec437ba2cfdb",
    "sink.webhook.level": "",
    "sink.webhook.namespaces": "",
    "sink.webhook.kinds": "",
    "sink.webhook.reason": "",
    "sink.webhook.method": "",
    "sink.webhook.header": "",
    "sink.webhook.use_custom_body": "false"
    }

    sink_cmds,app = enable_kube_eventer_catalog(answners)
    sink_cmd = "--sink=webhook:https://oapi.dingtalk.com/robot/send?access_token=4b4de1354b290410523dcdf3ab04a5b3775f799e8d54949e7952ec437ba2cfdb&"
    assert sink_cmd in sink_cmds
...
def test_sink_webhook_all():
    answners = {
    "sinktarget": "webhook",
    "sink.webhook.webhook_url": "https://oapi.dingtalk.com/robot/send?access_token=4b4de1354b290410523dcdf3ab04a5b3775f799e8d54949e7952ec437ba2cfdb",
    "sink.webhook.level": "info",
    "sink.webhook.namespaces": "default",
    "sink.webhook.kinds": "Pod,Node",
    "sink.webhook.reason": "test-webhook-all",
    "sink.webhook.method": "PUT",
    "sink.webhook.header": "Cookie=123",
    "sink.webhook.use_custom_body": "false"
    }

    sink_cmds,app = enable_kube_eventer_catalog(answners)
    sink_cmd = "--sink=webhook:https://oapi.dingtalk.com/robot/send?access_token=4b4de1354b290410523dcdf3ab04a5b3775f799e8d54949e7952ec437ba2cfdb&level=info&namespaces=default&kinds=Pod,Node&reason=test-webhook-all&method=PUT&header=Cookie=123"
    assert sink_cmd in sink_cmds
...
def enable_kube_eventer_catalog(answers, check=True):
    system = namespace['system']
    ...
    kube_eventer_wl = kube_eventer[0]
    app = None
    if check:
        kube_eventer_wl = wait_for_wl_to_active(system, kube_eventer[0], 10)
        assert kube_eventer_wl.state == "active"
        # app
        app = wait_for_app_to_active(system, 'kube-eventer')
        assert app.state == "active"
    else:
        app_data = system.list_app(name='kube-eventer').data
        app = app_data[0]
